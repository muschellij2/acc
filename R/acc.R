...
#' 
#' 
#' @param data Data which consists of two columns [TimeStamp,counts] (i.e.
#' accelerometer counts file read in by function readCounts)
...
#' specified to summarize data using only data from a single axis. If the
#' option 'vm' is used, the square root of the squared sum of counts from three
#' axes (i.e. \eqn{\sqrt{{x}^{2}+{y}^{2}+{z}^{2}}}) are used for the summary.
#' If the option 'sum' is used, sum of the counts from three axes are used.
...
#' patype is specified, boutsize should be specified for each one (e.g., if
...
#' @return \item{summary}{ Returns summary for each specified physical activity
#' types (number of bouts and minutes of the activity), for valid dates.}
